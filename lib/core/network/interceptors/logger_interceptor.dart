    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          debugPrint('║${_indent(tabs0)} $key: $value${!isLast ? ',' : ''}');
        } else {
          debugPrint('║${_indent(tabs0)} $key: {');
          _printPrettyMap(value, tabs: tabs0);
        }
      } else if (value is List) {
        if (compact && _canFlattenList(value)) {
          debugPrint('║${_indent(tabs0)} $key: ${value.toString()}');
        } else {
          debugPrint('║${_indent(tabs0)} $key: [');
          _printList(value, tabs: tabs0);
          debugPrint('║${_indent(tabs0)} ]${isLast ? '' : ','}');
        }
      } else {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          debugPrint('║${_indent(tabs0)} $key: $msg${!isLast ? ',' : ''}');
        }
      }
    });
